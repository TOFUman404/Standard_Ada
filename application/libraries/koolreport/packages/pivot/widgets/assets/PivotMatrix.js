...
KoolReport.newPivotMatrix = KoolReport.newPivotMatrix || (function(global){
    ...
    var pivotFunctions = (function() {
        ...
        var update = function(func) {
            ...
            oReq.open('POST', global.location.href);
        ...
        var initPivot = function(initProp) {
            ...
            var fieldDrops = pivotEl.querySelectorAll('.krpmFieldDrop');
        ...
        var expandCollapseRow = function (icon) {
        ...
        var expandCollapseColumn = function (icon) {
            if (! this.batchOperation)
                this.saveViewstate();
            var iconName = icon.className;
            var expand = iconName.indexOf('plus') > -1;
            ...
            var nodes = [td.dataset.node];
            var columnIndex = td.dataset.columnIndex;
                ...
                for (var i = 0; i < children.length; i += 1) {
                    ...
                    if (columnIndex === rangeRight - 1) { //column in All group
                        child.colSpan = expand ? 1 
                            : (firstColumnInAll ? td.colSpan - numDf + 1 : 1);
        ...
        var collapseTree = function(zone, expandTree, level, td) {
            ...
            if (level === 0) {
                ...
                numDf = numDf > 0 ? numDf : 1;
                var rangeRight = rangeLeft + 1*td.colSpan / numDf;
                var tmpTr = td.parentElement.nextElementSibling;
                if (tmpTr)
                    for (var i=0; i<tmpTr.children.length; i+=1) {
                        var child = tmpTr.children[i];
                        var columnIndex = 1*child.dataset.columnIndex;
            ...
            for (var i=0; i<childrenEls.length; i+=1) {
                ...
                if (! childTree || (childTree && childTree.collapse)) {
                    ...
                    var icon = childrenEl.firstElementChild;
